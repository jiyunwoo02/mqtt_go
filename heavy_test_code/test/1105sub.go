...
// MQTT 메시지 수신 함수
func subscribeToMQTT(client mqtt.Client, topic string, id int, wg *sync.WaitGroup, results chan<- SubscriberResult, stopChan chan bool) {
	defer wg.Done()
	...
	client.Subscribe(countTopic, 2, func(_ mqtt.Client, msg mqtt.Message) {
		...
		if count, err := strconv.Atoi(countPayload); err == nil {
			expectedCount += count // 기대되는 수신 수 설정
	...
	client.Subscribe(topic, 2, func(_ mqtt.Client, msg mqtt.Message) {
		select {
		case <-stopChan:
			return // stopChan에 신호가 오면 메시지 수신 중단
		default:
