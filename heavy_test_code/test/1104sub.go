...
func main() {
	...
	totalReceived := 0
	successfulCount := 0
	unsuccessfulSubscribers := []int{} // int 타입 요소를 가진 빈 슬라이스
	...
	fmt.Printf("성공적으로 수신한 메시지: %d/%d (%.1f%%)\n\n", *sn*totalReceived, *sn*totalMessages, (float64(totalReceived)/float64(totalMessages))*100)
